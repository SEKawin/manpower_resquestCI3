<!-- modal Manpower Requisition Form -->
<div class="modal fade" id="modal_manp_form_hrm_mgr" role="dialog">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title-hrm-mgr">Manpower Requisition Form</h3>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body form" id="append-modal-hrm-mgr">
            </div> <!-- modal-body form -->
...
<div id="hidden-modal-body-hrm-mgr" style="display: none;">
    <form action="#" id="form_manp_hrm_mgr" class="form-horizontal">
...
<script type="text/javascript">
    function form_approval_by_hrm_mgr(hr_no) {
        $('#append-modal-hrm-mgr').html($('#hidden-modal-body-hrm-mgr').html());
        $('body').find('#form_manp_hrm_mgr')[0].reset(); // reset form on modals
        $('.form-row').removeClass('has-error'); // clear error class
        $('#modal_manp_form_hrm_mgr').modal('show'); // show bootstrap modal
        $('.modal-title-hrm-mgr').text('แบบฟอร์มใบขออนุมัติอัตรากำลัง/ใบขอกำลังคนในงบประมาณ'); // Set Title to Bootstrap modal title
    ...
    function approval_hrm_mgr() {
        $('#btn_approval_hrm_mgr').text('กำลังทำการยืนยัน...'); //change button text
        $('#btn_approval_hrm_mgr').attr('disabled', true); //set button disable

        var formdata = $('#form_manp_hrm_mgr').serialize();
        // console.log(formdata)
        $.ajax({
            type: 'POST',
            url: "<?php echo site_url('Approval_manp/appr_by_hrm_mgr') ?>",
            data: formdata,
            dataType: 'JSON',
            success: function(data) {
                $('#append-modal-hrm-mgr').html($('#hidden-modal-body-hrm-mgr').html());
                $('#tbl_hrm_mgr').DataTable().ajax.reload(null, false);
                $('body').find('#form_manp_hrm_mgr')[0].reset(); // reset form on modals
                $('#modal_manp_form_hrm_mgr').modal('hide');
                alert('คุณทำการอนุมัติเรียบร้อยแล้ว');

                $('#btn_approval_hrm_mgr').text('ยืนยัน'); //change button text
                $('#btn_approval_hrm_mgr').attr('disabled', false); //set button enable
            },
            error: function(jqXHR, textStatus, errorThrown) {
