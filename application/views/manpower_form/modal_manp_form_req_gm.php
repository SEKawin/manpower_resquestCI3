...
<div class="modal fade" id="modal_manp_form_req_gm" role="dialog">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body form" id="append-modal_req_gm">
            </div> <!-- modal-body form -->
...
<div id="hidden-modal-body_req_gm" style="display: none;">
    <form action="#" id="form_manp_req_gm" class="form-horizontal">
        ...
        <div class="form-body">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-12">
                    <div class="input-group mb-3" id="approving_gm">

                        <?php foreach ($name_role as $row) :
                            $name_role = $row->name_role; ?>
                            <?php if ($name_role == 'GM') : ?>
                                <div id="approved_gm" style="display: none">
                                    <p>ผู้อนุมัติคำขอขั้นต้น <label name="name_gm"></label></p>
                                    <p>ผู้จัดการทั่วไป :<label name="status_gm"></label></p>
                                </div>
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="inputGroupSelect_gm">ผลการอนุมัติของผู้จัดการทั่วไป</label>
                                </div>
                                <select class="choice_gm custom-select" id="inputGroupSelect_gm" name="status_gm">
                                    <option selected>เลือก...</option>
                                    <option value="1">อนุมัติ</option>
                                    <option value="2">ไม่อนุมัติ</option>
                                </select>
                                <button type="button" class="btn btn-sm btn-primary" id="btn_approval_gm" onclick="approval_gm()">ยืนยัน</button>
                    </div>
...
<script type="text/javascript">
    function form_requested_gm(hr_no) {
        $('#append-modal_req_gm').html($('#hidden-modal-body_req_gm').html());
        $('body').find('#form_manp_req_gm')[0].reset(); // reset form on modals
        $('.form-row').removeClass('has-error'); // clear error class
        $('#modal_manp_form_req_gm').modal('show'); // show bootstrap modal
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data.data1, function(key, value) {
                    ...
                    if (value.status_gm == 1 || value.status_gm == 2) {
                        $('body').find('#approving_gm').hide('');
                        $('body').find('#approved_gm').show('');
                        ...
                        $('body').find('label[name="update_gm"]').text(value.update_mgr);
                    }
    ...
    function approval_gm() {
        $('#btn_approval_gm').text('กำลังทำการยืนยัน...'); //change button text
        $('#btn_approval_gm').attr('disabled', true); //set button disable

        var formdata = $('#form_manp_req_gm').serialize();
        // console.log(formdata)
        $.ajax({
            type: 'POST',
            url: "<?php echo site_url('Approval_manp/appr_by_gm') ?>",
            data: formdata,
            dataType: 'JSON',
            success: function(data) {
                $('#append-modal_req_gm').html($('#hidden-modal-body_req_gm').html());
                $('#tbl_req_gm').DataTable().ajax.reload(null, false);
                $('body').find('#form_manp_req_gm')[0].reset(); // reset form on modals
                $('#modal_manp_form_req_gm').modal('hide');
                alert('คุณทำการอนุมัติเรียบร้อยแล้ว');

                $('#btn_approval_gm').text('ยืนยัน'); //change button text
                $('#btn_approval_gm').attr('disabled', false); //set button enable
            },
            error: function(jqXHR, textStatus, errorThrown) {
