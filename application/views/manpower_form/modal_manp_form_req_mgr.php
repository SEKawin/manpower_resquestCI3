...
<div class="modal fade" id="modal_manp_form_req_mgr" role="dialog">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body form" id="append-modal_req_mgr">
            </div> <!-- modal-body form -->
...
<div id="hidden-modal-body_req_mgr" style="display: none;">
    <form action="#" id="form_manp_req_mgr" class="form-horizontal">
        ...
        <div class="form-body">
            ...
            <div class="form-row">
                ...
                <?php foreach ($name_role as $row) :
                    ...
                    <?php if ($name_role == 'DEPT_MGR') : ?>
                        <div class="form-group col-md-12">
                            <div class="input-group mb-3" id="approving_mgr">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="inputGroupSelect_mgr">ผลการอนุมัติของผู้จัดการฝ่าย</label>
                                </div>
                                <select class="choice_mgr custom-select" id="inputGroupSelect_mgr" name="status_dept_mgr">
                                    <option selected>เลือก...</option>
                                    <option value="1">อนุมัติ</option>
                                    <option value="2">ไม่อนุมัติ</option>
                                </select>
                                <button type="button" class="btn btn-sm btn-primary" id="btn_approval_mgr" onclick="approval_mgr()">ยืนยัน</button>
                            </div>
                            ...
                            <div id="approved_mgr" style="display: none">
...
<script type="text/javascript">
    function form_requested_mgr(hr_no) {
        $('#append-modal_req_mgr').html($('#hidden-modal-body_req_mgr').html());
        $('body').find('#form_manp_req_mgr')[0].reset(); // reset form on modals
        $('.form-row').removeClass('has-error'); // clear error class
        $('#modal_manp_form_req_mgr').modal('show'); // show bootstrap modal
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data.data1, function(key, value) {
                    ...
                    if (value.status_dept_mgr == 1 || value.status_dept_mgr == 2) {
                        $('body').find('#approving_mgr').hide('');
                        $('body').find('#approved_mgr').show('');
                        if (value.status_dept_mgr == 1) {
                            $('body').find('label[name="status_mgr"]').html(
                                '<span style="font-size: 15px;" class="badge badge-success">อนุมัติ</span>'
                            );
    ...
    function approval_mgr() {
        $('#btn_approval_mgr').text('กำลังทำการยืนยัน...'); //change button text
        $('#btn_approval_mgr').attr('disabled', true); //set button disable

        var formdata = $('#form_manp_req_mgr').serialize();
        $.ajax({
            type: 'POST',
            url: "<?php echo site_url('Approval_manp/appr_by_mgr') ?>",
            data: formdata,
            dataType: 'JSON',
            success: function(data) {
                $('#append-modal_req_mgr').html($('#hidden-modal-body_req_mgr').html());
                $('#tbl_req_mgr').DataTable().ajax.reload(null, false);
                $('body').find('#form_manp_req_mgr')[0].reset(); // reset form on modals
                $('#modal_manp_form_req_mgr').modal('hide');
                alert('คุณทำการอนุมัติเรียบร้อยแล้ว');

                $('#btn_approval_mgr').text('ยืนยัน'); //change button text
                $('#btn_approval_mgr').attr('disabled', false); //set button enable
            },
            error: function(jqXHR, textStatus, errorThrown) {
